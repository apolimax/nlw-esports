...
function CreateAdModal({ games }: CreateAdModalProps) {
  const [weekDays, setWeekDays] = useState<string[]>([]);
  const [useVoiceChannel, setUseCVoiceChannel] = useState(false);
  ...
  async function handleCreateAd(event: FormEvent) {
    ...
    try {
      await axios.post(`http://localhost:3500/games/${data.game}/ads`, {
        name: data.name,
          ...
          <div className="mt-2 flex gap-2 text-sm items-center">
            <Checkbox.Root
              ...
              onCheckedChange={(checked) => {
                if (checked) {
                  setUseCVoiceChannel(true);
                } else {
                  setUseCVoiceChannel(false);
                }
              }}
